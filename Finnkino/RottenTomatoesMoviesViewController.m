...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *currentMoviePosterURL = [[[self.allEvents objectAtIndex:indexPath.row]  posters] original];
    __block UIImage *myImage = nil;
    // Download image in the background thread
    dispatch_async(concurrentQueue, ^
                   {
                       // Retrieve the URL for image
                       NSURL *urlFromString = [[NSURL alloc] initWithString:currentMoviePosterURL];
    ...
    // Set the label text
    cellLabel.text = [[self.allEvents  objectAtIndex:indexPath.row] title];
    return cell;
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    {
        [rtdvc setValue:[self.allEvents objectAtIndex:indexPath.row] forKey:@"selection"];
    }
}
